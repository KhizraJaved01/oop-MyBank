...
class Bank {
    ...
    initialDataDisplayed: boolean = false;
    ...
    displayIndividualCustomer(accountNo: number) {
    ...
    withdraw(accountNo: number, amount: number) {
    ...
    deposit(accountNo: number, amount: number) {
        const index = this.accounts.findIndex(account => account.Account_No === accountNo);
        if (index !== -1) {
            this.accounts[index].Balance += amount;
            ...
            case "View All Customers":
                if (!myBank.initialDataDisplayed) {
                    ...
                    for (let i = 1; i <= 10; i++) {
                        ...
                        const newAccount: BankAccount = { Account_No: newCustomer.Account_No, Balance: Math.floor(Math.random() * 5000) + 1000 };
